...
userrouter.get("/profile", catchAsyncErrors(async (req, res, next) => {
...
userrouter.post("/add-address", catchAsyncErrors(async (req, res, next) => {
...
userrouter.get("/addresses", catchAsyncErrors(async (req, res, next) => {
  const { email } = req.query;
  if (!email) {
      return next(new ErrorHandler("Please provide an email", 400));
  }
