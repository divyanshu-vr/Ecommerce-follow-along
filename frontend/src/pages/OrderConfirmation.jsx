...
const OrderConfirmation = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const addressResponse = await axios.get('http://localhost:8080/api/v2/user/addresses', {
                    params: { email: email },
                });
                ...
                const cartResponse = await axios.get('http://localhost:8080/api/v2/product/cartproducts', {
                    params: { email: email },
                });
                ...
                const processedCartItems = cartData.cart.map(item => ({
                    ...
                    image: item.productId.images.map(imagePath => `http://localhost:8080${imagePath}`),
    ...
    const handlePlaceOrder = async (paymentType = 'cod', paypalOrderData = null) => {
        try {
            ...
            const response = await axios.post('http://localhost:8080/api/v2/orders/place-order', payload);
                        ...
                        {paymentMethod === 'paypal' && (
                             <div className='mt-4' style={{ maxWidth: '500px' }}>
                                 <PayPalScriptProvider
                                     options={{
                                         'client-id': 'AbZSSmDg5IzdRLLXsX7RPoyerfErWB6dyOYxxmWmUnRPFxPIP0jjV9xvm3QSTrjGaeNOmnawH87TAyWl', 
                                     }}
                                 >
                                     <PayPalButtons
